...
import { checkCardNumber } from '../../shared/utils';
...
export class NgValidatorExtendService {
  ...
  regex(reg: any): ValidatorFn {
    return (ctrl: AbstractControl) => {
      ...
      return !value || (value && regx.test(value))
        ? null
        : {
  ...
  /**
   * 与同级栏位进行日期比对，不得比另一个栏位小
   * @param  {string}   name 同级栏位名称
   * @return {ValidatorFn}      验证器
   */
  dateBigger(name: string, format?: string): ValidatorFn {
    return (ctrl: AbstractControl) => {
      let value = ctrl.value;
      if (!ctrl.parent) {
        return null;
      ...
      if (anotherVal) {
        if (new Date(format?moment(value).format(format): value).getTime() - new Date(format?moment(anotherVal).format(format): anotherVal).getTime() >= 0) {
          setTimeout(() => {
  ...
  /**
   * 与同级栏位进行日期比对，不得比另一个栏位大
   * @param  {string}   name 同级栏位名称
   * @return {ValidatorFn}      验证器
   */
  dateSmall(name: string,format?: string): ValidatorFn {
    return (ctrl: AbstractControl) => {
      let value = ctrl.value;
      if (!ctrl.parent) {
        return null;
  ...
  checkCardNumber() {
    return (ctrl: AbstractControl) => {
      let value = ctrl.value;
      if (!value) return null;
      const res = checkCardNumber(value);
      return res
        ? null
        : {
            cardNumber: true,
