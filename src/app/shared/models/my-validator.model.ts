...
export class MyValidatorModel {
  validators: any = {};
  constructor(
    target: { name: string; valiItems: ValiItem[] }[],
    bind: any = {},
  ) {
  ...
  addValidator(name: string, valiItem: ValiItem): void {
    ...
    this.validators[name].dataset[
      'v' + valiItem.valiName + 'Message'
    ] = valiItem.errMessage ? valiItem.errMessage : '';
  }
  addValidators(name: string, valiItem: ValiItem[]): void {
    for (let i = 0; i < valiItem.length; i++) {
      this.validators[name].dataset['v' + valiItem[i].valiName] =
        valiItem[i].valiValue;
      this.validators[name].dataset[
        'v' + valiItem[i].valiName + 'Message'
      ] = valiItem[i].errMessage ? valiItem[i].errMessage : '';
    }
  }
